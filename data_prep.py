...
class ScoreCard:
    ...
    def get_batting_points(self, pointsconfig) -> pd.DataFrame:
        """
        pointsconfig: dictionary with points per score type as per dream11
        :return:
        """
        batsmen_scorecard = self.batsmen_summary_fun()
        ...
        batsmen_scorecard['run_bonus_points'] = np.where(batsmen_scorecard['total_runs'] >= 50, pointsconfig['>=50'] +
                                                         batsmen_scorecard['run_bonus_points'],
                                                         batsmen_scorecard['run_bonus_points'])
        ...
        batsmen_scorecard['total_bat_points'] = batsmen_scorecard['total_runs_points'].add(
            batsmen_scorecard['run_6_points'], fill_value=0). \
            add(batsmen_scorecard['run_4_points'], fill_value=0).add(batsmen_scorecard['run_bonus_points'],
                                                                     fill_value=0)
        return batsmen_scorecard

    def get_bowling_points(self, pointsconfig) -> pd.DataFrame:
        """
        pointsconfig: dictionary with points per score type as per dream11
        :return:
        """
        bowler_scorecard = self.bowler_summary_fun()
        ...
        bowler_scorecard['economy_rate_points'] = np.where(bowler_scorecard['economy_rate'] >= 11, pointsconfig['>11E'],
                                                           np.where(bowler_scorecard['economy_rate'] >= 10,
                                                                    pointsconfig['>10E'],
                                                                    np.where(bowler_scorecard['economy_rate'] >= 9,
                                                                             pointsconfig['>9E'],
                                                                             np.where(
                                                                                 bowler_scorecard['economy_rate'] <= 4,
                                                                                 pointsconfig['<=4E'],
                                                                                 np.where(bowler_scorecard[
                                                                                              'economy_rate'] <= 5,
                                                                                          pointsconfig['<5E'],
                                                                                          np.where(bowler_scorecard[
                                                                                                       'economy_rate'] <= 6,
        ...
        bowler_scorecard['wicket_bonus_points'] = np.where(bowler_scorecard['total_wickets'] >= 5, pointsconfig['>=5W'],
                                                           np.where(bowler_scorecard['total_wickets'] >= 4,
                                                                    pointsconfig['>=4W'], 0))
        bowler_scorecard['total_bowl_points'] = 0
        bowler_scorecard['total_bowl_points'] = bowler_scorecard['total_wickets_points'].add(
            bowler_scorecard['maiden_overs_points'], fill_value=0). \
            add(bowler_scorecard['wicket_bonus_points'], fill_value=0).add(bowler_scorecard['economy_rate_points'],
                                                                           fill_value=0)
